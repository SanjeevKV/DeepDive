...
logging.basicConfig(filename='convnext_with_dev_full_size_direct_vid.log', format='%(asctime)s - %(message)s', level=logging.INFO)
...
def get_data_dict(file_loc):
	...
	data_dict = dict(map( lambda x : (get_vid_name(x['name']), x), data) )
	return data_dict	

def get_vid_names_from_keys(names):
	return list(map(get_vid_name, names))

def get_ref_tensor(data_dict, gp_dataset):
	ref_tensor_list = []
	for gp in gp_dataset:
		ref_tensor_list.append(data_dict[gp['name']]['sign'])
	return torch.cat(ref_tensor_list, dim=0)

def get_inp_tensor(gp_dataset):
	inp_tensor_list = []
	for gp in gp_dataset:
		inp_tensor_list.append(torch.from_numpy(gp['sign']))
	return torch.cat(inp_tensor_list, dim=0)
...
def train_one_epoch(data_dict, part_folder, raw_names, model, optimizer, device, bs = 8):
	...
	for i, f in enumerate(files):
		logging.info(f'Training group number {i}, {device}')
		gp_dataset = load_dataset_file(os.path.join(part_folder, f))
		for gp in gp_dataset:
			inp_tensor = get_inp_tensor([gp]).to(device)
			ref_tensor = get_ref_tensor(data_dict, [gp]).to(device)
			optimizer.zero_grad()
			...
			target = ref_tensor#out_ref#torch.randn(1,1024)
...
def evaluate_model(dev_dict, dev_folder, dev_raw_names, model, optimizer, device):
	...
	model.eval()
	total_loss = 0
	files = os.listdir(dev_folder)
	for i, f in enumerate(files):
		#logging.info(f'Training video number {i}, {device}')
		optimizer.zero_grad()
		gp_dataset = load_dataset_file(os.path.join(dev_folder, f))
		for gp in gp_dataset:
			inp_tensor = get_inp_tensor([gp]).to(device)
			ref_tensor = get_ref_tensor(dev_dict, [gp]).to(device)
			# cur_vid = prepare_video_from_folder(os.path.join(part_folder, get_vid_name(rn))).to(device)
			# out_ref = data_dict[rn]['sign'].to(device)
...
def main():
	...
	base_images_folder = '/scratch1/maiyaupp/phoenix/phoenix_direct_vid'
