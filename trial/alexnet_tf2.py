"""
source: https://raw.githubusercontent.com/neccam/nslt/master/nslt/alexnet.py
...
def conv(x, filter_height, filter_width, num_filters, stride_y, stride_x, name, padding='SAME', groups=1):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        #weights = tf.Variable(name='weights', shape=[filter_height,
                                                    filter_width,
...
def fc(x, num_in, num_out, name, relu=True):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        #weights = tf.Variable(name='weights', shape=[num_in, num_out],
                                  trainable=True)
        biases = tf.compat.v1.get_variable('biases', [num_out], trainable=True)
        #biases = tf.Variable(name='biases', shape=[num_out], trainable=True)
...
def max_pool(x, filter_height, filter_width, stride_y, stride_x, name, padding='SAME'):
    ...
    return tf.nn.max_pool2d(input=x, ksize=[1, filter_height, filter_width, 1], strides=[1, stride_y, stride_x, 1], padding=padding, name=name)
...
def dropout(x, keep_prob, name):
    ...
    return tf.nn.dropout(x, 1 - (keep_prob), name=name)
